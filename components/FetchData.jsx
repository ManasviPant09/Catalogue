  ...
  return (
    <div className="flex flex-col justify-center mt-0.5 w-full mr-2 bg-gray-200">
        ...
        {paginatedPokemons.map((pokemon) => (
          ...
          >
            <img
              src={pokemon.image}
              alt={pokemon.name}
              className="rounded-md w-full h-52 object-contain mx-auto p-4 rounded-lg bg-white"
            />
            <div
              key={pokemon.id}
              className="bg-gray-200 pr-2 pl-2 pb-2 rounded-md"
            >
              <h2 className="text-lg font-bold text-gray-700 mt-2 mb-1">
                #0{pokemon.number}
              </h2>
              <h2 className="text-lg font-bold text-gray-700 mt-2 mb-1">
                {pokemon.name}
              </h2>
              <p className="text-white rounded-md bg-gray-700 p-2">
                {pokemon.types.join(', ')}
              </p>
            </div>
          </div>
      ...
      <div className="flex justify-center mt-4 mb-4">
        {paginatedPokemons.length < (data?.pokemons?.length ?? 0) && (
          <button
            className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-2 rounded rounded-lg w-32"
            onClick={loadMore}
          >
            Load More
          </button>
